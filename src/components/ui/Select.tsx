...
const Select = ({ label, options = [], useSelect, icon, className }: SelectProps) => {
        ...
        <div className='flex flex-col gap-y-2'>
            ...
            <Listbox value={selected} onChange={handleSelect}>
                <div className="relative">
                    <Listbox.Button className={`relative w-full ${className} bg-white py-2 px-4
                    ${selected.value === null ? "text-slate-400" : "text-slate-500"}
                     text-left border border-slate-300 focus:outline-none sm:text-sm`} >
                        {icon ? <span className='absolute top-1/2 -translate-y-1/2'>
                            <Icon icon={icon} className="text-slate-400 text-xl" />
                        </span> : ''}
